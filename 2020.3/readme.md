适用于2020.3及以前的版本 食用规范

1. 下载2020.3及以前的版本
2. 安装点击试用
3. 启动软件->试用 (Evaluate for free) -> Evaluate，进入欢迎界面 ﹂插件(Plugins)->齿轮按钮->通过磁盘安装(Install Plugin From Disk)->浏览 ﹂选中本地激活文件插件BetterIntelliJ.jar，然后重启软件
4. 重启软件后﹂界面帮助(Help)->管理许可证(Manage License)-> 新增 (Add New License) ﹂选择激活码方式(Activation Code)，粘贴该激活激活码，点击激活
